...
#define TM_TAKTOS_TICK_CLOCK_HZ    SystemCoreClock
...
{
    // IRQ28 above PendSV/SysTick so the ISR runs first, then PendSV tail-chains.
...
extern "C" void tm_initialize(void (*test_initialization_function)(void))
{
    ...
    if (TaktOSInit(TM_TAKTOS_TICK_CLOCK_HZ, TM_TAKTOS_TICK_HZ, TAKTOS_TICK_CLOCK_PROCESSOR, 0) != TAKTOS_OK) {
...
// -----------------------------------------------------------------------------
// UART output  blocking via tm_uart_send.
// Reports print every 30 s so blocking time is irrelevant to benchmark accuracy.
// -----------------------------------------------------------------------------
