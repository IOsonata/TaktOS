...
// -----------------------------------------------------------------------------
// ThreadX bare-metal STM32H753ZI port for the official Thread-Metric harness.
//
// Target: NUCLEO-H753ZI, STM32H753ZIT6, Cortex-M7 + DP FPU @ 64 MHz (HSI).
// UART  : USART3 on PD8 (AF7), 115200 8N1 (in tm_console_nucleo_h753.cpp - shared).
...
// from ports/cortex_m7/gnu/src/tx_thread_*.S via the __tx_SysTickHandler etc.
// strong symbols, which override the weak aliases in startup_stm32h753.S at
// link time.
// -----------------------------------------------------------------------------

#define TM_THREADX_MAX_THREADS       9     /* matches TaktOS MAX_SLOTS on this board */
...
// -----------------------------------------------------------------------------
// NVIC addresses. STM32H7 has 4 implementable priority bits.
// TIM7_IRQn = 55 on STM32H753xx. Pended only - TIM7 is never clocked.
// -----------------------------------------------------------------------------
...
#define SW_IRQ_N                     55u
/* 0xC0 matches the TaktOS H753 port's SW-IRQ priority for comparable runs. */
...
// Called from _tx_initialize_kernel_enter.
...
void tm_thread_sleep(int seconds)   { tx_thread_sleep((ULONG)(seconds * TM_THREADX_TICK_HZ)); }
void tm_thread_sleep_ticks(int t)   { tx_thread_sleep((ULONG)(t <= 0 ? 1 : t)); }
