...
#define TM_TAKTOS_CORE_CLOCK_HZ    SystemCoreClock
...
void tm_initialize(void (*test_initialization_function)(void))
{
    ...
    if (TaktOSInit(TM_TAKTOS_CORE_CLOCK_HZ, TM_TAKTOS_TICK_HZ, TAKTOS_TICK_CLOCK_PROCESSOR) != TAKTOS_OK) {
...
static void tm_uart_flush_async(void)
{
    if (g_uart_tx_idx > 0)
    {
        tm_uart_send(g_uart_tx_buf, g_uart_tx_idx);
