...
static StaticSemaphore_t g_mutex_ctrl[TM_RTOS_MAX_MUTEXES];
static SemaphoreHandle_t g_mutexes[TM_RTOS_MAX_MUTEXES];
...
{
    if (mutex_id < 0 || mutex_id >= TM_RTOS_MAX_MUTEXES)
        return TM_ERROR;

    g_mutexes[mutex_id] = xSemaphoreCreateMutexStatic(&g_mutex_ctrl[mutex_id]);
    g_mutex_created[mutex_id] = (g_mutexes[mutex_id] != NULL);
...
{
    if (mutex_id < 0 || mutex_id >= TM_RTOS_MAX_MUTEXES ||
        !g_mutex_created[mutex_id])
        return TM_ERROR;
    return (xSemaphoreTake(g_mutexes[mutex_id], portMAX_DELAY) == pdTRUE) ? TM_SUCCESS : TM_ERROR;
}
...
int tm_mutex_unlock(int mutex_id)
{
    if (mutex_id < 0 || mutex_id >= TM_RTOS_MAX_MUTEXES ||
        !g_mutex_created[mutex_id])
        return TM_ERROR;
    return (xSemaphoreGive(g_mutexes[mutex_id]) == pdTRUE) ? TM_SUCCESS : TM_ERROR;
}
