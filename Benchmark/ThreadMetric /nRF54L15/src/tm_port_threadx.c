...
typedef struct TmThreadDesc_s {
    ...
    void      (*entry)(void);
    TX_THREAD   thread;
    ULONG       stack[TM_RTOS_STACK_BYTES / sizeof(ULONG)];
} TmThreadDesc_t;

static TmThreadDesc_t  g_threads[TM_RTOS_MAX_THREADS];
static TX_QUEUE      g_queues[TM_RTOS_MAX_QUEUES];
static ULONG         g_queue_storage[TM_RTOS_MAX_QUEUES][TM_RTOS_QUEUE_DEPTH * TM_RTOS_QUEUE_ULONGS_PER_MSG];
static bool          g_queue_created[TM_RTOS_MAX_QUEUES];
static TX_SEMAPHORE  g_semaphores[TM_RTOS_MAX_SEMAPHORES];
static bool          g_semaphore_created[TM_RTOS_MAX_SEMAPHORES];
static TX_MUTEX      g_mutexes[TM_RTOS_MAX_MUTEXES];
static bool          g_mutex_created[TM_RTOS_MAX_MUTEXES];
static void        (*g_test_init)(void) = NULL;

static inline UINT tm_to_threadx_priority(int tm_priority)
{
    if (tm_priority < 1)  tm_priority = 1;
    if (tm_priority > 31) tm_priority = 31;
    return (UINT)(tm_priority - 1);
}

static void tm_thread_trampoline(ULONG arg)
{
    int id = (int)arg;
...
{
    g_test_init = test_initialization_function;
    for (int i = 0; i < TM_RTOS_MAX_THREADS; ++i) {
        g_threads[i].allocated = false;
        g_threads[i].tm_priority = 31u;
        g_threads[i].entry = NULL;
    }
    for (int i = 0; i < TM_RTOS_MAX_QUEUES; ++i) g_queue_created[i] = false;
    for (int i = 0; i < TM_RTOS_MAX_SEMAPHORES; ++i) g_semaphore_created[i] = false;
    for (int i = 0; i < TM_RTOS_MAX_MUTEXES; ++i) g_mutex_created[i] = false;
    for (int i = 0; i < TM_RTOS_MAX_POOLS; ++i) { g_pool_created[i] = false; g_pool_free[i] = NULL; }
...
int tm_thread_create(int thread_id, int priority, void (*entry_function)(void))
{
    ...
    UINT s = tx_thread_create(&t->thread, (CHAR*)"tm", tm_thread_trampoline, (ULONG)thread_id,
                              t->stack, sizeof(t->stack),
                              tm_to_threadx_priority(priority), tm_to_threadx_priority(priority),
                              TX_NO_TIME_SLICE, TX_DONT_START);
    return (s == TX_SUCCESS) ? TM_SUCCESS : TM_ERROR;
}

int tm_thread_resume(int thread_id)
{
    if (thread_id < 0 || thread_id >= TM_RTOS_MAX_THREADS || !g_threads[thread_id].allocated) return TM_ERROR;
    return (tx_thread_resume(&g_threads[thread_id].thread) == TX_SUCCESS) ? TM_SUCCESS : TM_ERROR;
}

int tm_thread_suspend(int thread_id)
{
    if (thread_id < 0 || thread_id >= TM_RTOS_MAX_THREADS || !g_threads[thread_id].allocated) return TM_ERROR;
    return (tx_thread_suspend(&g_threads[thread_id].thread) == TX_SUCCESS) ? TM_SUCCESS : TM_ERROR;
}
...
{
    tx_thread_sleep((ULONG)((seconds <= 0) ? 0u : ((uint32_t)seconds * TM_RTOS_TICK_HZ)));
}
...
int tm_queue_create(int queue_id)
{
    ...
    UINT s = tx_queue_create(&g_queues[queue_id], (CHAR*)"tmq", TM_RTOS_QUEUE_ULONGS_PER_MSG,
                             g_queue_storage[queue_id], sizeof(g_queue_storage[queue_id]));
    g_queue_created[queue_id] = (s == TX_SUCCESS);
...
int tm_queue_send(int queue_id, unsigned long *message_ptr)
{
    ...
    return (tx_queue_send(&g_queues[queue_id], message_ptr, TX_NO_WAIT) == TX_SUCCESS) ? TM_SUCCESS : TM_ERROR;
}

int tm_queue_receive(int queue_id, unsigned long *message_ptr)
{
    if (queue_id < 0 || queue_id >= TM_RTOS_MAX_QUEUES || !g_queue_created[queue_id] || message_ptr == NULL) return TM_ERROR;
    return (tx_queue_receive(&g_queues[queue_id], message_ptr, TX_NO_WAIT) == TX_SUCCESS) ? TM_SUCCESS : TM_ERROR;
}

int tm_semaphore_create(int semaphore_id)
{
    if (semaphore_id < 0 || semaphore_id >= TM_RTOS_MAX_SEMAPHORES) return TM_ERROR;
    UINT s = tx_semaphore_create(&g_semaphores[semaphore_id], (CHAR*)"tms", 1u);
    g_semaphore_created[semaphore_id] = (s == TX_SUCCESS);
...
int tm_semaphore_get(int semaphore_id)
{
    ...
    return (tx_semaphore_get(&g_semaphores[semaphore_id], TX_NO_WAIT) == TX_SUCCESS) ? TM_SUCCESS : TM_ERROR;
}

int tm_semaphore_put(int semaphore_id)
{
    if (semaphore_id < 0 || semaphore_id >= TM_RTOS_MAX_SEMAPHORES || !g_semaphore_created[semaphore_id]) return TM_ERROR;
    return (tx_semaphore_put(&g_semaphores[semaphore_id]) == TX_SUCCESS) ? TM_SUCCESS : TM_ERROR;
}

int tm_mutex_create(int mutex_id)
{
    if (mutex_id < 0 || mutex_id >= TM_RTOS_MAX_MUTEXES) return TM_ERROR;
    UINT s = tx_mutex_create(&g_mutexes[mutex_id], (CHAR*)"tmm", TX_INHERIT);
    g_mutex_created[mutex_id] = (s == TX_SUCCESS);
...
int tm_mutex_lock(int mutex_id)
{
    ...
    return (tx_mutex_get(&g_mutexes[mutex_id], TX_WAIT_FOREVER) == TX_SUCCESS) ? TM_SUCCESS : TM_ERROR;
}

int tm_mutex_unlock(int mutex_id)
{
    if (mutex_id < 0 || mutex_id >= TM_RTOS_MAX_MUTEXES || !g_mutex_created[mutex_id]) return TM_ERROR;
    return (tx_mutex_put(&g_mutexes[mutex_id]) == TX_SUCCESS) ? TM_SUCCESS : TM_ERROR;
}
