...
#define TM_MUTEX_NUM_WORKERS  4   /* threads 0–3, all TM priority 10 */
...
void tm_mutex_reporter(void)
{
    ...
    TM_REPORT_LOOP
    {
        ...
        /* Keep arithmetic in uint64_t before cast — min_delta*1000 can exceed
         * ULONG_MAX (4 294 967 295) when ops/window > ~4.3M (e.g. TaktOS).
         * Casting first then dividing gives a wrong result (overflow). */
        unsigned long fairness = (max_delta > 0u)
            ? (unsigned long)(min_delta * (uint64_t)1000u / max_delta)
            : 1000u;   /* e.g. 998 means 0.998 */
