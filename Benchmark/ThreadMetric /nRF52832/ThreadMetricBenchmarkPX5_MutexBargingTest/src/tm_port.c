...
static pthread_mutex_t         g_tm_mutex[TM_PX5_MAX_MUTEXES];
static bool                    g_tm_mutex_created[TM_PX5_MAX_MUTEXES];
...
void tm_initialize(void (*test_initialization_function)(void))
{
    ...
    for (i = 0; i < TM_PX5_MAX_MUTEXES; ++i) g_tm_mutex_created[i] = false;
...
{
    tick_t t = (tick_t)(ticks <= 0 ? 1 : ticks);
    (void)px5_pthread_tick_sleep(t);
}
...
{
    if (mutex_id < 0 || mutex_id >= TM_PX5_MAX_MUTEXES)
        return TM_ERROR;
    if (pthread_mutex_init(&g_tm_mutex[mutex_id], NULL) != PX5_SUCCESS)
        return TM_ERROR;
    g_tm_mutex_created[mutex_id] = true;
    return TM_SUCCESS;
}

int tm_mutex_lock(int mutex_id)
{
    if (mutex_id < 0 || mutex_id >= TM_PX5_MAX_MUTEXES || !g_tm_mutex_created[mutex_id])
        return TM_ERROR;
    return (pthread_mutex_lock(&g_tm_mutex[mutex_id]) == PX5_SUCCESS) ? TM_SUCCESS : TM_ERROR;
}
...
int tm_mutex_unlock(int mutex_id)
{
    if (mutex_id < 0 || mutex_id >= TM_PX5_MAX_MUTEXES || !g_tm_mutex_created[mutex_id])
        return TM_ERROR;
    return (pthread_mutex_unlock(&g_tm_mutex[mutex_id]) == PX5_SUCCESS) ? TM_SUCCESS : TM_ERROR;
}
