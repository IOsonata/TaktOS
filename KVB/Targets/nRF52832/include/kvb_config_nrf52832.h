/**-------------------------------------------------------------------------
 * @file    kvb_config_nrf52832.h
 * @brief   KVB build configuration for nRF52-DK (PCA10040) + TaktOS.
 *
 ...
 *
 * Memory budget (64 KB SRAM total) — nRF52832 has roughly 8x the SRAM of
 ...
 *   Total                                                    ~= 7.0 / 64.0 KB
 *
 * Stacks here are larger than the STM32F0308 build because Cortex-M4 has a
 * 32-bit ARMv7E-M context (8 callee-save + 8 hardware-stacked = 16 words
 ...
 * -------------------------------------------------------------------------*/
#ifndef KVB_CONFIG_NRF52832_H
#define KVB_CONFIG_NRF52832_H
 ...
 * on a 64 KB SRAM target — holds ~22 ms of TX at 115200 baud, more than
 * enough headroom for any single kvb_logf line.  Same value across all
 * three nRF52832 kernel variants. */
